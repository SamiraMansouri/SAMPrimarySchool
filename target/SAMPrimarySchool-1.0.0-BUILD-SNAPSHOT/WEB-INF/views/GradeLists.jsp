			...
			<div class="col-md-9">
				<c:if test="${not empty gradeLists}">
					<table class="table-bordered table-hover table">
						...
						<tbody>
							<c:forEach var="gradeList" items="${gradeLists }">
								<tr>
									<!-- checkbox -->
									<td>
										<div class="checkbox">
											<label><input class="checkbox" type="checkbox"
												name="case" id="${gradeList.id}"></label>
										</div>
									</td>
									<!-- checkbox -->

									<!-- subject -->
									<td><label>${gradeList.subject}</label></td>
									<!-- subject -->

									<!-- scale -->
									<td><label>${gradeList.scale}</label></td>
									<!-- scale -->

									<!-- description -->
									<td><label>${gradeList.description}</label></td>
									<!-- description -->

									<!-- date -->
									<td><label>${gradeList.date}</label></td>
									<!-- date -->

									<!-- show gradeList -->
									<td><a href="ShowGradeList.htm?glId=${gradeList.id}">Show</a></td>
									<!-- show Assignment -->
					...
					<div>
						<button type="button" class="btn  btn-danger btn-lg pull-right"
							id="delete" onclick="return confirmation();"
							style="display: none;">delete</button>
					</div>


				</c:if>
				<c:if test="${empty gradeLists }">
					<div class="panel panel-default">
